...
class State(int, Enum):
    GENERAL = 1      # handle brackets, EOF, \n, Spaces(\t, \r, \s), Equal(=) and Comma(,)
    IDENTIFIER = 2   # handle function name (e.g. Nav, Link ...) and Variable names (new_nav = ....)
...
class Tokenizer: 
    ...
    def handle_string(self, char):
        if char == "\n" or char == None:
            raise InvalidTokenError("Expected a \" for termination while new-line or null terminated character was read")
        elif char == "\"":
    ...
    def handle_number(self, char):
        if self.is_number_end(char):
            ...
            raise InvalidTokenError(f"Expected a number while invalid character [{char}] was read")
    ...
    def handle_identifier(self, char):
        if self.is_identifier_end(char):
            ...
            raise InvalidTokenError(f"Expected a number, letter or underscore while invalid character [{char}] was read")
